...
| 独立筛选页与排序 | `PARTIAL` | 当前结果为静态演示数据，排序未真正作用于商品集合 | [#6](../../issues/6) |
| 商品详情 | `PARTIAL` | 正常商品可展示；无效 ID 仍可能回退到第一条数据 | [#1](../../issues/1) |
| 学生认证 | `LOCAL_DEMO` / `PARTIAL` | 不发送验证码，不构成真实身份认证；输入校验仍需收紧 | [#8](../../issues/8) |
| 商品发布 | `PARTIAL` | 标题、价格和成色可写入内存；分类、描述、图片、地点和成功页尚未完整连接 | [#4](../../issues/4) |
| 议价与结算 | `LOCAL_DEMO` / `PARTIAL` | 可展示出价和结算路径；成交金额尚未形成完整订单快照 | [#3](../../issues/3) |
| 订单支付与核销 | `LOCAL_DEMO` / `PARTIAL` | 正常演示路径为 `wait_pay → wait_meetup → completed`；当前尚未严格限制非法状态跳转 | [#2](../../issues/2) |
| 商品与订单状态同步 | `PARTIAL` | 订单完成后商品状态尚未可靠切换为已售 | [#3](../../issues/3) |
| 会话列表与本地消息追加 | `LOCAL_DEMO` | 消息仅保存在当前运行内存中 | — |
| 收藏 | `PARTIAL` | 首页、详情页和收藏页尚未共享同一收藏状态 | [#5](../../issues/5) |
| 卖家主页 | `PARTIAL` | 当前页面使用固定卖家数据，尚未按 `seller_id` 加载 | [#7](../../issues/7) |
| 校园社区互动 | `LOCAL_DEMO` | 发帖、点赞、关注和分享为本地交互演示 | — |
| 钱包、充值与提现 | `UI_ONLY` / `LOCAL_DEMO` | 不产生资金流动 | — |
| 图片上传、GPS 定位 | `NOT_IMPLEMENTED` | 页面中存在演示入口或静态展示 | [#4](../../issues/4) |
...
该命令不是完整测试套件，目前不会覆盖所有错误参数、非法订单状态、跨页面状态同步或 UI 回归。扩展测试与 GitHub Actions 由 [#9](../../issues/9) 跟踪。
...
- 无效实体 ID 的错误处理尚待 [#1](../../issues/1) 完成。

## Backlog 与实施顺序

当前工作已拆分为可独立关闭的 Issue：

1. [#1：拒绝无效商品、订单和会话 ID](../../issues/1)
2. [#2：约束订单状态迁移](../../issues/2)
3. [#3：保存成交快照并同步商品状态](../../issues/3)
4. [#4：打通发布表单和成功页](../../issues/4)
5. [#5：统一收藏状态](../../issues/5)
6. [#6：使用真实商品数据实现筛选](../../issues/6)
7. [#7：按卖家 ID 加载卖家主页](../../issues/7)
8. [#8：收紧 Demo 认证输入验证](../../issues/8)
9. [#9：增加失败路径测试和 GitHub Actions](../../issues/9)
10. [#10：统一文档事实与维护来源](../../issues/10)
